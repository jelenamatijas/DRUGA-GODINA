...
import net.etfbl.pricalo.clientRadnik.ClientRadnik;
...
//    }
    ...
    while (!ClientRadnik.stop && !logedout)
    {
      readLine();
    }//zatvoren while
        ...
        if ((s = in.readLine()) != null) 
        {
          ...
          if (!s.startsWith("MSG")&&!s.startsWith("TELEEKRAN"))
          {
            if ("##STOP##".equalsIgnoreCase(s))
            {
              ...
              ClientThreadRadnik.logedout = true;
              ClientRadnik.ref.interrupt();
              this.interrupt();
            }
            else
            {
