...
import net.etfbl.pricalo.server.Server;
import net.etfbl.pricalo.server.interfejsi.ReportManagement.*;
import net.etfbl.pricalo.server.izvjestaj.*;
...
public class ServerThread extends Thread implements ReportManagement
{
    ...
    boolean  back = false;
      ...
      try
      {
        ...
        do
        {
          ...
          if("LOGIN".equalsIgnoreCase(str)) back = login();
          
          else if ("REG".equalsIgnoreCase(str)) registration();
          else if ("LOGOUT".equalsIgnoreCase(str)) logedout = logout();
          else if ("NADZORNIK".equalsIgnoreCase(str))
          {
            System.out.println("NADZORNIK se prijavio.");
            do
            {
              Server.aktivniRadnici.put("NADZORNIK",this);
              this.username = new String("NADZORNIK");
              Server.onlineClientsList.add("NADZORNIK");
              str = in.readLine();
              if("1".equalsIgnoreCase(str))searchByJMB();
              else if ("2".equalsIgnoreCase(str))searchByKeyword();
              else if ("3".equalsIgnoreCase(str))makeReport();
              else if ("4".equalsIgnoreCase(str))readReport();
              else if ("5".equalsIgnoreCase(str))
              {
                String m = new String();
                m = in.readLine();
                ServerThread tempPrep = Server.aktivniRadnici.get("PREPRAVLJAC");
                if(tempPrep != null)
                {
                  tempPrep.forward(m);
                  out.println("SENT");
                }
                else out.println("TELEEKRAN : PREPRAVLJAC nije trenutno dospupan.");
              }
              else if (str.equalsIgnoreCase("LOGOUT")) logedout = logoutNadzornik();
            }while(!logedout);
          }
          else if ("PREPRAVLJAC".equalsIgnoreCase(str))
          {
            System.out.println("PREPRAVLJAC se prijavio.");
            Server.aktivniRadnici.put("PREPRAVLJAC",this);
            Server.onlineClientsList.add("PREPRAVLJAC");
            username = new String("PREPRAVLJAC");
            do
            {
              str = in.readLine();
          ...
          else if ("AGENCIJA".equalsIgnoreCase(str))
          {
            System.out.println("AGENCIJA je povezana.");
            Server.aktivniRadnici.put("AGENCIJA",this);
            Server.onlineClientsList.add("AGENCIJA");
            do
            {
             
              str = in.readLine();
              if("1".equalsIgnoreCase(str))makeReport();
              else if ("2".equalsIgnoreCase(str))readReport();
              else if ("LOGOUT".equalsIgnoreCase(str)) logedout = logoutAgencija();
            }
            while(!logedout);
          }
          else if ("SIMULATION".equalsIgnoreCase(str)) simulation();
          else if (str != null) send(str);
        } while ((!"STOP".equalsIgnoreCase(str) || back) && !logedout);
      ...
      catch (SocketException se)
      {
        if(username != null)
        {
          System.out.println("Prekinuta je veza sa klijentom " + username );
          Server.aktivniRadnici.remove(username);
        }
        else System.out.println("Prekinuta veza sa klijentom");
        try
        {
          in.close();
          out.close();
        ...
        {
          System.out.println("Greska pri zatvaranju stream-ova zbog prekinute veze.");
        }
      }
      catch (Exception ex)
      {
              ...
              synchronized (Server.radniciRegistar)
              {
                ...
                if (usrOK)
                {
                  if ((str.split("#")[1]).equalsIgnoreCase(user.split("#")[1]))
                  {
                    ...
                    out.println("LOGINOK");
                    Server.onlineClientsList.add(username);
                    return false;
                  }
                  else
                  {
  ...
  public synchronized void send (String s)
  {
    ...
    if ((temp = Server.aktivniRadnici.get(s.split("#")[1])) != null)
    {
      ...
      try
      {
        f = new File("net" + File.separator + "etfbl" +File.separator+"pricalo"+File.separator+"server"+File.separator+"messages");
        ...
        if (fileList != null && !"".equalsIgnoreCase(search))
        {
          ...
          if (fileCounter != 0) 
          {
            ...
            if (number != 0 && number <= fileCounter)
            {
              ...
              {
                 BufferedReader readFile = new BufferedReader(new FileReader("net" + File.separator + "etfbl" + File.separator + "pricalo" + File.separator + "server" + File.separator
  ...
  public synchronized void searchByKeyword()
  {
    try
    {
      ...
      File f = new File("net" + File.separator + "etfbl" + File.separator + "pricalo" + File.separator + "server" + File.separator + "messages");
        ...
        if (fileList != null && !"".equalsIgnoreCase(search))
        {
          ...
          if (fileCounter != 0) 
          {
            ...
            fileForDownload = in.readLine();
                 ...
                 File fileForReplace = new File ("net" + File.separator + "etfbl" + File.separator + "pricalo" + File.separator + "server" + File.separator
                                                                            + "messages" + File.separator + fileForDownload);
  ...
  public boolean logoutPrepravljac()
  {
    ...
    return false;
  }//zatvorena funkcija logoutPrepravljac
  
  
   public boolean logoutAgencija()
  {
    out.println("LOGOUTOK");
    ...
    try
    {
      ...
      out.close();
      this.sock.close();
      System.out.println("AGENCIJA se uspjesno odjavio.");
      return true;
    }
    catch(IOException ioex)
    {
    ...
    return false;
  }//zatvorena funkcija logoutAgencija
  ...
  public void readReport()
  {
    ...
    File[] fileList = null;
    String namesToStream = new String();
    namesToStream = "";
    try
    {
      if (f.exists() && f.isDirectory())
      {
        ...
        if (fileList != null)
        {
          ...
          out.println(namesToStream);
          String input = new String();
          
          input = in.readLine();
          int k = Integer.valueOf(input);
          
          input = fileNames.get(k);
          Izvjestaj izv = new Izvjestaj();
          izv =(Izvjestaj) Izvjestaj.read(input);
    ...
    {
      System.out.println("Greska pri citanju sa streama pri ucitavanju izvjestaja.");
    }
    catch(Exception ex)
    {
      ex.printStackTrace();
  ...
  public synchronized void simulation()
  {
    String command = new String();
    try
    {
    command = in.readLine();
    if ("STOP".equalsIgnoreCase(command)) simulationStop();
  ...
  }//zatvorena funkcija simulation
  
  public synchronized void simulationStop()
  {
     try
    {
      ...
      InetAddress ipTemp = InetAddress.getByName("127.0.0.1");
      
      Socket tempSock = new Socket(ipTemp,9000);
      tempSock.close();
      
       for(String s : Server.aktivniRadnici.keySet())
      {
        //String s = new String();
        //s = Server.onlineClientsList.get(i);
        //System.out.println("S je " + s);
        
        ServerThread tempServerThread = Server.aktivniRadnici.get(s);
        //Server.aktivniRadnici.remove(s);
        if("NADZORNIK".equalsIgnoreCase(s))
        {
          tempServerThread.forward("##STOP##");
          //tempServerThread.closeStreams();
        }
        else if ("PREPRAVLJAC".equalsIgnoreCase(s))
        {
          tempServerThread.forward("##STOP##");
         // tempServerThread.closeStreams();
        }
        else if ("AGENCIJA".equalsIgnoreCase(s))
        {
          tempServerThread.forward("##STOP##");
          //tempServerThread.closeStreams();
        }
      ...
      String info = new String();
      out.println(Server.radniciRegistar.size());
      
      for (String k : Server.radniciRegistar.keySet())
      {
        Radnik temp = (Radnik) Server.radniciRegistar.get(k);
        info += temp.getIme();
        info += " ";
        info += temp.getPrezime();
        info += " ";
        info += temp.getJMB();
        info += " ";
        info += temp.getPlata();
